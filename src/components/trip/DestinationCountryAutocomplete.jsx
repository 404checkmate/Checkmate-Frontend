...
export default function DestinationCountryAutocomplete({
  ...
  return (
    <div ref={comboRef} className="relative z-20">
      <div
        className={`relative border border-teal-100/80 bg-white shadow-inner transition-[border-radius,box-shadow] ${
          isPanelOpen ? 'rounded-t-2xl ring-2 ring-teal-200' : 'rounded-2xl'
        }`}
      >
        ...
        <div className="flex items-stretch gap-1 pr-1">
          <input
            ...
            } ${countryInputReadOnly ? 'cursor-pointer bg-teal-50/40' : ''}`}
          />
          {countryInputReadOnly && onChangeCountryRequest && (
            <button
              type="button"
              onClick={onChangeCountryRequest}
              className="my-2 shrink-0 self-center rounded-lg px-2.5 py-1 text-xs font-semibold text-teal-700 underline-offset-2 hover:bg-teal-100/80 hover:underline"
            >
              국가 변경
            </button>
          )}
      ...
      {isPanelOpen && (
        <div
          ...
          className={`absolute left-0 right-0 top-full z-30 overflow-y-auto rounded-b-2xl border border-t-0 border-teal-200 bg-white shadow-lg ring-2 ring-teal-200 ring-t-0 ${panelMaxHeight}`}
        >
          {!isArrivalStep && (
            <>
                  ...
                  {suggestions.map((c) => (
                    <li key={c.name} role="none">
                      <button
                        ...
                        className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-gray-800 transition hover:bg-teal-50"
                      >
          ...
          {isArrivalStep && (
            <div className="border-t border-teal-100">
              <div className="sticky top-0 z-10 border-b border-teal-100 bg-teal-50/95 px-3 py-2">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-teal-800/90">취항지 검색</p>
                <p className="text-xs text-gray-600">도시명 또는 공항 코드(IATA)로 찾을 수 있어요.</p>
              </div>
              <div className="relative border-b border-teal-50 bg-white px-3 py-2">
                <TripDestinationSvgIcon
                  name="search"
                ...
                <input
                  ...
                  autoComplete="off"
                  className="w-full rounded-xl border border-teal-100 bg-white py-2.5 pl-10 pr-3 text-sm text-gray-800 outline-none ring-0 focus:border-teal-300"
                />
              </div>
