const china = {
  "code": "china",
  "flag": "🇨🇳",
  "name": "중국",
  "cities": [
    "베이징",
    "상하이",
    "충칭",
    "칭다오"
  ],
  "photos": {
    "hero": "https://images.unsplash.com/photo-1538428494232-9c0d8a3ab403?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8Y2hpbmElMjBjaXR5fGVufDB8fDB8fHww",
    "sections": [
      "https://images.unsplash.com/photo-1561845376-11c89062fdff?q=80&w=1332&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1743384257019-692097518687?q=80&w=932&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1532300481631-0bc14f3b7699?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1579860876104-66a737d50772?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1586784444981-ac96e335555c?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1544449566-dee13645ade8?q=80&w=1169&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "https://images.unsplash.com/photo-1538428494232-9c0d8a3ab403?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8Y2hpbmElMjBjaXR5fGVufDB8fDB8fHww"
    ]
  },
  "hero": {
    "title": "중국 여행\n완벽 준비 가이드",
    "subtitle": ""
  },
  "sections": [
    {
      "id": "climate",
      "kicker": "Climate & Packing",
      "icon": "",
      "title": "기후 & 옷차림",
      "body": "중국은 국토가 매우 넓어 지역에 따라 기후 차이가 큰 편입니다. 방문 도시의 날씨를 미리 확인하는 것이 중요합니다.<br/>베이징·시안(북부)은 한국과 비슷하게 사계절이 뚜렷하지만, 겨울에는 체감 온도가 영하 20도까지 떨어져 매우 춥고 건조합니다. 상하이(동부), 청두·충칭(내륙), 광저우(남부) 등은 겨울에도 비교적 온화한 편이지만, 여름에는 매우 덥고 습해 체감온도가 한국보다 높게 느껴질 수 있습니다.<br/>기본적으로 방문 시기에 맞는 옷차림을 준비하면 되지만, 실내 냉방이 강한 편이므로 얇은 겉옷 하나를 챙기는 것을 추천합니다.",
      "photo": "https://images.unsplash.com/photo-1561845376-11c89062fdff?q=80&w=1332&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": {
        "icon": "💡",
        "body": "중국은 지역별 날씨 차이가 매우 큰 나라예요! 같은 시기라도 도시마다 기온 차이가 크니 출발 전 날씨를 꼭 확인하세요."
      }
    },
    {
      "id": "money",
      "kicker": "Money & Pay",
      "icon": "",
      "title": "환전 & 결제",
      "body": "중국 화폐는 위안(CNY, 元/RMB)으로, 100위안은 한화 약 2만원입니다. 중국은 대부분의 상점과 식당에서 알리페이(Alipay) 또는 위챗페이(WeChat Pay) 기반 QR 결제를 사용하며, 일부 매장에서는 현금이나 해외 신용카드 사용이 제한될 수 있습니다. 최근에는 카카오페이·네이버페이·토스페이에서 알리페이플러스와 연계하여 결제가 가능하니 미리 준비해가시는 것을 추천합니다. (다만, 결제 건당 0.5달러의 수수료가 발생하는 경우가 있으니 주의가 필요합니다)<br/>현금은 비상용으로 소액(소액권 위주)만 환전하면 충분하며, ATM 인출이 필요하면 은련(UnionPay) 로고가 있는 곳을 이용하고, 트래블월렛·트래블로그 등 해외 결제 카드도 보조 수단으로 챙겨두면 안심입니다.",
      "photo": "https://images.unsplash.com/photo-1743384257019-692097518687?q=80&w=932&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": {
        "icon": "💡",
        "body": "알리페이 교통카드를 발급해 놓으면 지하철, 버스 모두 편하게 사용할 수 있어요!"
      }
    },
    {
      "id": "health",
      "kicker": "Health & Safety ",
      "icon": "",
      "title": "건강 & 안전",
      "body": "중국은 전반적으로 치안이 좋은 편이지만, 관광지나 번화가에서는 소매치기와 바가지 요금에 주의해야 합니다. 수돗물은 바로 마시지 않는 것이 좋으며, 생수를 구매해 마시는 것을 추천합니다. 또한 지역에 따라 향신료가 강하거나 기름진 음식이 많아 평소 장이 예민하다면 소화제와 지사제를 준비하는 것이 좋습니다. 차량과 전동스쿠터가 신호를 무시하고 달리는 경우가 많으니 횡단보도에서도 항상 좌우를 살피며 건너는 것이 좋습니다.",
      "photo": "https://images.unsplash.com/photo-1532300481631-0bc14f3b7699?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": {
        "icon": "💡",
        "body": "베이징 등 북부 도시는 대기오염이 심한 날이 있어 호흡기가 예민하다면 마스크를 챙기세요!"
      }
    },
    {
      "id": "apps",
      "kicker": "Must-Have Apps",
      "icon": "",
      "title": "필수 앱",
      "body": "중국은 모바일 결제와 앱 기반 서비스가 생활 전반에 자리 잡고 있어 관련 앱을 미리 준비하면 여행이 훨씬 편리해집니다.",
      "photo": "https://images.unsplash.com/photo-1579860876104-66a737d50772?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": {
        "icon": "💡",
        "body": "중국에서는 일부 지역에서 구글 서비스 사용이 원활하지 않을 수 있어요! "
      }
    },
    {
      "id": "phrasebook",
      "kicker": "Language & Culture",
      "icon": "",
      "title": "기본 회화",
      "body": "중국어(표준 중국어, 보통화)는 성조가 있어 발음이 까다롭지만, 간단한 인사 몇 마디만 해도 현지인이 반갑게 대해줍니다. 영어가 잘 통하지 않는 곳이 많아 기본 표현과 번역 앱을 함께 활용하면 좋습니다.<br/>• 안녕하세요: 你好 (니하오)<br/>• 감사합니다: 谢谢 (시에시에)<br/>• 실례합니다 / 죄송합니다: 不好意思 (부하오이쓰)<br/>• 얼마예요?: 多少钱? (뚜오샤오첸?)<br/>• 고수 빼주세요: 不要香菜 (부야오 샹차이)<br/>• 화장실 어디예요?: 洗手间在哪里? (시쇼우젠 짜이 날리?)",
      "photo": "https://images.unsplash.com/photo-1586784444981-ac96e335555c?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": null
    },
    {
      "id": "notes",
      "kicker": "What to Know",
      "icon": "",
      "title": "기타 주의사항",
      "body": "<strong>비자</strong><br/>한국인은 일반적으로 관광 목적 방문 시 무비자 입국 정책 적용 여부를 출국 전 반드시 확인해야 합니다. 정책이 수시로 변경될 수 있으므로 최신 정보를 확인하세요.<br/><strong>인터넷 검열</strong><br/>구글·인스타그램·카카오톡 등 다수 서비스가 차단되므로 VPN을 미리 준비해야 합니다.<br/><strong>생활 문화</strong><br/>가격 흥정이 가능한 시장에서는 부르는 값보다 낮춰 협상하는 것이 일반적입니다. 일부 호텔은 외국인 투숙이 제한될 수 있으니 예약 시 '외국인 투숙 가능' 여부를 확인하는 것이 좋습니다.<br/><strong>촬영주의</strong><br/>공안(경찰), 군사시설, 정부 청사 등은 촬영이 금지될 수 있으니 주의하세요.",
      "photo": "https://images.unsplash.com/photo-1544449566-dee13645ade8?q=80&w=1169&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      "tip": {
        "icon": "💡",
        "body": "최근 VPN 기능이 포함된 e-SIM, u-SIM이 많으니 미리 준비해가는 것을 추천드려요!"
      }
    }
  ],
  "apps": [
    {
      "name": "Alipay (알리페이)",
      "iconUrl": "https://www.google.com/s2/favicons?domain=alipay.com&sz=128",
      "emoji": "💳",
      "desc": "중국에서 가장 널리 사용되는 모바일 결제 앱입니다. 해외 카드 등록이 가능하며 결제, 대중교통 이용 등에 활용할 수 있습니다.",
      "storeUrl": "alipay.com/"
    },
    {
      "name": "고덕지도(Amap)",
      "iconUrl": "https://www.google.com/s2/favicons?domain=amap.com&sz=128",
      "emoji": "🚶‍♀️",
      "desc": "중국 현지에서 가장 많이 사용하는 지도 앱입니다. 중국 내 길 찾기와 대중교통 검색에 유용합니다.",
      "storeUrl": "amap.com/"
    },
    {
      "name": "DiDi (디디)",
      "iconUrl": "https://www.google.com/s2/favicons?domain=didiglobal.com&sz=128",
      "emoji": "🚗",
      "desc": "중국 최대 차량 호출 앱입니다. 한국의 카카오택시와 비슷하며, 택시 호출 시 가장 많이 사용됩니다.",
      "storeUrl": "didiglobal.com/"
    },
    {
      "name": "美团 (메이퇀)",
      "iconUrl": "https://www.google.com/s2/favicons?domain=meituan.com&sz=128",
      "emoji": "🛵",
      "desc": "음식 배달, 맛집 검색, 관광지 입장권 예약, 액티비티 예약까지 가능한 중국 대표 생활 플랫폼입니다.",
      "storeUrl": "meituan.com/"
    },
    {
      "name": "大众点评 (따중뎬핑)",
      "iconUrl": "https://www.google.com/s2/favicons?domain=dianping.com&sz=128",
      "emoji": "🍽️",
      "desc": "맛집 검색, 레스토랑 예약, 웨이팅에 가장 많이 사용됩니다.",
      "storeUrl": "dianping.com/"
    }
  ],
  "checklist": [
    {
      "cat": "🗺️ 현지 정보",
      "prepType": "etc",
      "section": [
        "climate"
      ],
      "items": [
        "지역별 날씨 확인"
      ]
    },
    {
      "cat": "👕 의류",
      "prepType": "clothing",
      "section": [
        "climate"
      ],
      "items": [
        "레이어드용 얇은 겉옷"
      ]
    },
    {
      "cat": "💳 금융 & 환전",
      "prepType": "pre_booking",
      "section": [
        "money"
      ],
      "items": [
        "현금 소액 환전"
      ]
    },
    {
      "cat": "💳 금융 & 환전",
      "prepType": "essentials",
      "section": [
        "money"
      ],
      "items": [
        "여행용 카드 or QR앱 준비"
      ]
    },
    {
      "cat": "🛡️ 안전",
      "prepType": "pre_booking",
      "section": [
        "health"
      ],
      "items": [
        "여행자 보험 가입"
      ]
    },
    {
      "cat": "💊 건강 & 의약품",
      "prepType": "health",
      "section": [
        "health"
      ],
      "items": [
        "지사제"
      ]
    },
    {
      "cat": "💊 건강 & 의약품",
      "prepType": "health",
      "section": [
        "health"
      ],
      "items": [
        "소화제"
      ]
    },
    {
      "cat": "💊 건강 & 의약품",
      "prepType": "health",
      "section": [
        "health"
      ],
      "items": [
        "마스크"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "etc",
      "section": [
        "apps"
      ],
      "items": [
        "Alipay 앱 설치"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "etc",
      "section": [
        "apps"
      ],
      "items": [
        "Amap(고덕지도) 앱 설치"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "etc",
      "section": [
        "apps"
      ],
      "items": [
        "DiDi(디디추싱) 앱 설치"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "etc",
      "section": [
        "apps"
      ],
      "items": [
        "美团 (메이퇀) 앱 설치"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "etc",
      "section": [
        "apps"
      ],
      "items": [
        "大众点评 (따중뎬핑) 앱 설치"
      ]
    },
    {
      "cat": "🤝 현지 문화",
      "prepType": "etc",
      "section": [
        "notes"
      ],
      "items": [
        "비자 정책 확인"
      ]
    },
    {
      "cat": "📱 앱 & 전자기기",
      "prepType": "essentials",
      "section": [
        "notes"
      ],
      "items": [
        "VPN 포함된 이심 / 유심 구매"
      ]
    },
    {
      "cat": "🤝 현지 문화",
      "prepType": "etc",
      "section": [
        "notes"
      ],
      "items": [
        "호텔 외국인 투숙 가능 여부 확인"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "essentials",
      "section": [],
      "items": [
        "항공권 (e티켓 캡처·출력본)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "essentials",
      "section": [],
      "items": [
        "여권 / 여권 복사본"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "essentials",
      "section": [],
      "items": [
        "볼펜"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "여벌옷"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "잠옷"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "속옷"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "양말"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "편한 신발"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "모자"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "clothing",
      "section": [],
      "items": [
        "선글라스"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "health",
      "section": [],
      "items": [
        "감기약"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "health",
      "section": [],
      "items": [
        "해열진통제"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "health",
      "section": [],
      "items": [
        "연고/소독약"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "health",
      "section": [],
      "items": [
        "알코올 스왑"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "health",
      "section": [],
      "items": [
        "밴드"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "toiletries",
      "section": [],
      "items": [
        "칫솔 / 치약"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "toiletries",
      "section": [],
      "items": [
        "세안용품 (클렌징폼·클렌징오일·립앤아이 리무버)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "toiletries",
      "section": [],
      "items": [
        "샤워용품 (샴푸·린스·바디워시)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "toiletries",
      "section": [],
      "items": [
        "면봉 / 화장솜"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "toiletries",
      "section": [],
      "items": [
        "면도기 / 제모기"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "스킨케어"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "자외선 차단제"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "색조 화장품"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "헤어용품(머리끈·머리핀·헤어롤)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "헤어기기(헤어드라이기, 고데기)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "beauty",
      "section": [],
      "items": [
        "헤어제품(에센스·오일·왁스·스프레이)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "electronics",
      "section": [],
      "items": [
        "보조배터리"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "electronics",
      "section": [],
      "items": [
        "충전기"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "electronics",
      "section": [],
      "items": [
        "이어폰"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "travel_goods",
      "section": [],
      "items": [
        "휴지/물티슈"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "travel_goods",
      "section": [],
      "items": [
        "양우산"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "travel_goods",
      "section": [],
      "items": [
        "보조가방"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "travel_goods",
      "section": [],
      "items": [
        "샤워기 필터"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_booking",
      "section": [],
      "items": [
        "항공권 예약"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_booking",
      "section": [],
      "items": [
        "숙소 예약"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "여권 만료일 (6개월 이상)"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "온라인 체크인"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "체크인 마감시간"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "카운터 위치"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "캐리어 무게"
      ]
    },
    {
      "cat": "😉 기본 준비물",
      "prepType": "pre_departure_check",
      "section": [],
      "items": [
        "수하물 규정"
      ]
    }
  ],
  "footerCta": {
    "title": "나만의 중국 여행 체크리스트, 지금 바로 만들어보세요 🗺️",
    "subtitle": "AI가 여행 날짜·동행·스타일을 분석해 딱 맞는 준비물을 추천해드려요"
  }
}

export default china
